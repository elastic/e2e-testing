...
# Build OP Binary
GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} make -C metricbeat fetch-binary

# Build runtime dependencies
STACK_VERSION=${STACK_VERSION} make -C metricbeat run-elastic-stack

# Sync integrations
make -C metricbeat sync-integrations
...
set +e
exit_status=0
if ! FLAG=${FLAG} FEATURE=${FEATURE} FORMAT=junit STACK_VERSION=${STACK_VERSION} METRICBEAT_VERSION=${METRICBEAT_VERSION} make -C metricbeat functional-test | tee ${REPORT}  ; then
