...
func Attach(ctx context.Context, deployer deploy.Deployer, service deploy.ServiceRequest, action string, actionOpts interface{}) (deploy.ServiceOperation, error) {
	...
	case ActionWaitForProcess:
		...
		attachAction := AttachActionWaitProcess(deployer, service, newActionOpts)
