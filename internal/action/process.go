...
func AttachActionWaitProcess(deployer deploy.Deployer, service deploy.ServiceRequest, actionOpts ProcessAction) deploy.ServiceOperation {
...
func runInContainer(ctx context.Context, a *actionWaitProcess) (string, error) {
	...
	processStatus := func() error {
		...
		output, err := a.deployer.ExecIn(ctx, deploy.NewServiceRequest(common.FleetProfileName), a.service, cmds)
		if err != nil {
		...
		for _, pid := range pids {
			...
			pidState, err := a.deployer.ExecIn(ctx, deploy.NewServiceRequest(common.FleetProfileName), a.service, pidStateCmds)
			if err != nil {
				log.WithFields(log.Fields{
					"cmds":          cmds,
					"desiredState":  a.opts.DesiredState,
