...
func TestGetGCPBucketCoordinatesForProject_Commits(t *testing.T) {
	artifact := "elastic-agent"

	t.Run("Fetching Elastic Agent commits bucket for RPM package", func(t *testing.T) {
		GithubCommitSha1 = "0123456789"
		defer func() { GithubCommitSha1 = "" }()

		fileName := "elastic-agent-" + testVersion + "-x86_64.rpm"

		bucket, prefix, object := getGCPBucketCoordinatesForProject("elastic-agent", fileName, artifact)
		assert.Equal(t, bucket, "beats-ci-artifacts")
	...
	t.Run("Fetching Elastic Agent commits bucket for ubi8 Docker image", func(t *testing.T) {
		...
		bucket, prefix, object := getGCPBucketCoordinatesForProject("elastic-agent", fileName, "elastic-agent-ubi8")
		assert.Equal(t, bucket, "beats-ci-artifacts")
		assert.Equal(t, prefix, "elastic-agent/commits/0123456789")
...
func TestGetGCPBucketCoordinates_ForProjectSnapshots(t *testing.T) {
	artifact := "elastic-agent"

	t.Run("Fetching Elastic Agent snapshots bucket for RPM package", func(t *testing.T) {
		fileName := "elastic-agent-" + testVersion + "-x86_64.rpm"
