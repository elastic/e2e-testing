...
Scenario: Adding the Endpoint Integration to an Agent makes the host to show in Security App
  ...
  When the "Elastic Endpoint Security" integration is "added" in the "default" policy
  Then the "Elastic Endpoint Security" datasource is shown in the "default" policy as added
...
Scenario: Removing Endpoint from Agent policy stops the connected Endpoint
  Given an Endpoint is successfully deployed with a "centos" Agent
  When the "Elastic Endpoint Security" integration is "removed" in the "default" policy
