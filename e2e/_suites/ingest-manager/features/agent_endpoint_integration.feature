...
Scenario: Adding the Endpoint Integration to an Agent makes the host to show in Security App
  ...
  When the "Elastic Endpoint Security" integration is "added" in the "default" configuration
  Then the "Elastic Endpoint Security" datasource is shown in the "default" configuration as added
...
Scenario: Removing Endpoint from Agent configuration stops the connected Endpoint
  Given an Endpoint is successfully deployed with a "centos" Agent
  When the "Elastic Endpoint Security" integration is "removed" in the "default" configuration
